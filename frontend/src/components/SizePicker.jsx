...
function SizePicker({ onSizePick }) {
  return (
    <div className='px-4 mt-8 mb-12 divide-y-2 divide-gray-300 lg:px-0 lg:mt-0 divide-solid'>
      ...
      <ul className='grid grid-cols-5 gap-4 px-4 py-4 sm:grid-cols-9 lg:grid-cols-3 justify-items-center '>
        {sizeArr.map((size, i) => (
          <li
            className='flex items-center justify-center w-14 h-10 lg:w-10 lg:h-8 text-center border-[1.4px] border-solid rounded border-customBrown transition duration-150 hover:scale-110 hover:cursor-pointer'
            key={i}
